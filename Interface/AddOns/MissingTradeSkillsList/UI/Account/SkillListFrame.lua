...
MTSLACCUI_SKILL_LIST_FRAME = {
    ...
    -- Sort by name by default (2 for by level)
    current_sort = 1,
    ...
    Initialise = function(self, parent_frame)
        ...
        self.ui_frame:SetPoint("BOTTOMLEFT", MTSLACCUI_PROFESSION_LIST_FRAME.ui_frame, "BOTTOMRIGHT", 0, 1)
        -- Create the buttons
        self.SKILLS_BUTTONS = {}
        local left = 6
        local top = -6
    ...
    ----------------------------------------------------------------------------------------------------------
    ChangeProfession = function(self, trade_skill_name, missing_skills)
        ...
        if self.trade_skill_name ~= trade_skill_name then
            ...
            -- get sort flag of sort frame
            self.current_sort = MTSLACCUI_SKILL_LIST_SORT_FRAME.current_sort
            self.profession_skills = MTSL_TOOLS:GetAllKnownSkillsForProfession(trade_skill_name, missing_skills, self.current_sort, MTSLACCUI_SKILL_LIST_SORT_FRAME.current_cont_zone)
    ...
    SortSkills = function(self, sort_by_name)
        ...
        if self.current_sort ~= sort_by_name and self.profession_skills ~= nil then
    ...
    HandleSelectedListItem = function(self, id)
        ...
        if self.selected_button_index ~= id then
            ...
            MTSLACCUI_SKILL_DETAIL_FRAME:ShowDetailsOfSkill(self.trade_skill_name, selected_skill)
