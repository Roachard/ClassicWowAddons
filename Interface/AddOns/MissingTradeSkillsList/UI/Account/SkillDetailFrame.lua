...
MTSLACCUI_SKILL_DETAIL_FRAME = {
    ...
    Initialise = function (self, parent_frame)
        ...
        -- position just left of the skill List Frame
        self.ui_frame:SetPoint("BOTTOMLEFT", MTSLACCUI_SKILL_LIST_FRAME.ui_frame, "BOTTOMRIGHT", 0, 0)
        --  Black background
        ...
        self.labels.name.tooltip_frame:SetScript("OnEnter", function() MTSLACCUI_SKILL_DETAIL_FRAME:ToolTipShowSkillName() end)
        ...
        self.labels.type.tooltip_frame:SetScript("OnEnter", function() MTSLACCUI_SKILL_DETAIL_FRAME:ToolTipShowSourceName() end)
        ...
        self.labels.sources.ui_frame:SetScript("OnMouseUp", function() MTSLACCUI_SKILL_DETAIL_FRAME:AddTomTomWayPointPrimarySource() end)
        -- hide on creation
        ...
        self.labels.alt_type.tooltip_frame:SetScript("OnEnter", function() MTSLACCUI_SKILL_DETAIL_FRAME:ToolTipShowAltSourceName() end)
        ...
        -- add on click for tom tom integration
        self.labels.alt_sources.ui_frame:SetScript("OnMouseUp", function() MTSLACCUI_SKILL_DETAIL_FRAME:AddTomTomWayPointSecondarySource() end)
    end,
    ...
    ---------------------------------------------------------------------------
    ShowDetailsOfSkill = function(self, prof_name, skill)
        if skill ~= nil then
            ...
            if skill.min_skill <= 0 or skill.min_skill == nil then
                self.labels.min_skill.value:SetText(MTSLUI_FONTS.COLORS.TEXT.NORMAL .. "-")
            else
                self.labels.min_skill.value:SetText(MTSLUI_FONTS.COLORS.TEXT.NORMAL .. skill.min_skill)
            end
            -- Set minimum xp level
            self:SetRequiredXPLevel(skill.min_xp_level)
            ...
            if skill.item ~= nil then
                self:ShowDetailsOfSkillTypeItem(skill.item, prof_name)
    ...
    SetRequiredXPLevel = function (self, min_xp_level)
        ...
        if min_xp_level ~= nil then
            self.labels.requires_xp.value:SetText(MTSLUI_FONTS.COLORS.TEXT.NORMAL .. min_xp_level)
        else
    ...
    ShowDetailsOfSkillTypeTrainer = function(self, trainers_info)
        ...
        local trainers = MTSL_TOOLS:GetNpcsByIds(trainers_info.sources)
        -- Show all the npcs
    ...
    ShowDetailsOfSkillTypeQuest = function(self, quest_ids, is_alternative_source)
        ...
        local quest = MTSL_TOOLS:GetQuestByIds(quest_ids)
        if is_alternative_source == 1 then
        ...
        if quest ~= nil then
            ...
            if amount_npcs > 0 then
                ...
                local questgivers = MTSL_TOOLS:GetNpcsByIds(quest.npcs)
            ...
            if amount_items > 0 then
               -- local items = MTSL_TOOLS:GetItemsCurrentTradeSkillByIds(quest.items)
            ...
            if quest.objects ~= nil then
                local objects = MTSL_TOOLS:GetObjectsByIds(quest.objects)
    ...
    ShowDetailsOfSkillTypeObject = function (self, oject_id)
        local object = MTSL_TOOLS:GetObjectById(oject_id)
    ...
    ----------------------------------------------------------------------------------------------------
    ShowDetailsOfSkillTypeItem = function(self, item_id, prof_name)
        local item = MTSL_TOOLS:GetItemTradeSkillById(item_id, prof_name)
        if item  == nil then
            print(MTSLUI_FONTS.COLORS.TEXT.ERROR .. "MTSL - Count not find item with id " .. item_id .. " for profession " .. MTSL_CURRENT_TRADESKILL.NAME .. ". Please report this bug!")
        else
            ...
            if amount_sources <= 0 then
                ...
                if has_vendors > 0 then
                    ...
                    local vendors = MTSL_TOOLS:GetNpcsByIds(item.vendors.sources)
                ...
                if has_drops > 0 then
                    ...
                    if has_vendors <= 0 and has_quests <= 0 then
                        ...
                        if item.drops.mobs_range ~= nil then
                            ...
                            local mobs = MTSL_TOOLS:GetMobsByIds(item.drops.mobs)
            ...
            for i=1,amount_labels do
                if npcs[i] ~= nil then
                    ...
                    if npcs[i].reputation ~= nil then
                        text = text .. " [Requires ".. MTSL_TOOLS:GetFactionNameById(npcs[i].reputation.faction_id) .. " - " .. MTSL_TOOLS:GetReputaionLevelById(npcs[i].reputation.level_id) .. "]"
                    end
