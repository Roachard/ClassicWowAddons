...
MTSLUI_MTSLF_MISSING_SKILLS_LIST_FRAME = {
    ...
    Initialise = function(self, parent_frame)
        ...
        self.ui_frame:SetPoint("TOPLEFT", MTSLUI_MTSLF_MISSING_SKILLS_LIST_SORT_FRAME.ui_frame, "BOTTOMLEFT", 0, -7)
        -- Create the buttons
        self.SKILLS_BUTTONS = {}
        local left = 6
        local top = -6
    ...
    UpdateList = function (self)
        ...
        -- No need for slider
        if self.amount_missing_skills <= self.MAX_ITEMS_SHOWN then
            self.slider_active = 0
            self.ui_frame.slider:Hide()
        else
            self.slider_active = 1
            local max_steps = self.amount_missing_skills - self.MAX_ITEMS_SHOWN + 1
